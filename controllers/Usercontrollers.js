...
class data {
   static async createUser(req, res, next){
      try {
        const { email, password, name, createdAt, lastname, telephone } = req.body;
        ...
        res
          .status(200)
          .json({ message: "User Enregister", success: true, user });
        next();
      } catch (error) {
        console.error(error);
      }
   ...
   static async updateUser(req,res){
    try {
      const {id} = req.params
       ...
       if(!Person){
          res.status(404).json({message: `peut pas modifier l'element qui a l'id ${id}`})
       }
       res.status(200).json(Person)
    } catch (error) {
       res.status(500).json({message: message.error})
   ...
   static async deleteUser(req,res){
    try {
      const {id} = req.params
       ...
       if(!Person){
          res.status(404).json({message: `L'element qui a lid ${id} existe pas`})
       }
       res.status(200).json(Person)
    } catch (error) {
       res.status(500).json({message: message.error})
